...
export default function GridManager({mailer, setMailer, divisions, gridContainer, children}){
    const resizeMailer = () => {
        ...
        data.grid.w = data.grid.w == undefined ? res : data.grid.w < window.innerWidth ? window.innerWidth : data.grid.w;
        data.grid.h = data.grid.h == undefined ? res : (data.grid.h < window.innerHeight ? window.innerHeight : data.grid.h);
        setMailer(data);
    }
